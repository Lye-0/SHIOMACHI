...
for (let cursor = 0; cursor < queue.length; cursor++) {
  ...
  for (let i = 0; i < blocks.length; i++)
    for (let next = 0; next <= 6 - blocks[i].length; next++) {
...
for (let i = 0; i < breadth.length; i++) {
  ...
  for (const edge of adjacency.get(at))
    if (!distance.has(edge.key)) {
      distance.set(edge.key, distance.get(at) + 1);
      predecessor.set(edge.key, at);
      breadth.push(edge.key);
    }
}
const candidates = [...distance]
  .filter(([k, d]) => d >= 8 && d <= 13 && states.get(k)[0] === 0)
  .sort((a, b) => b[1] - a[1] || a[0].localeCompare(b[0]));
const selected = candidates[0] ?? [...distance].sort((a, b) => b[1] - a[1])[0];
let cursor = selected[0];
const solution = [];
while (predecessor.has(cursor)) {
  const next = predecessor.get(cursor);
  solution.push(adjacency.get(cursor).find((e) => e.key === next));
  cursor = next;
}
console.log(
  JSON.stringify(
    {
      states: states.size,
      position: states.get(selected[0]),
      minimumMoves: selected[1],
      solution,
    },
    null,
    2,
